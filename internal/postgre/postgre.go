...
func (s *Storage) List(limit, offset int64) ([]RequestFields, int64, error) {
	const op = "internal.postgre.List"
	slog.Info("Start list tx", slog.String("op", op))

	tx, err := s.db.Begin()
	if err != nil {
		return nil, 0, fmt.Errorf("%s: failed to begin tx: %w", op, err)
	}
	defer rollback(tx, op)
	...
	rows, err := tx.Query(`
		SELECT service_name, price, user_id, start_date, end_date
		FROM subscriptions
		ORDER BY service_name, user_id
		LIMIT $1 OFFSET $2;
	`, limit, offset)
	if err != nil {
		return nil, 0, fmt.Errorf("%s: failed to query rows: %w", op, err)
	}
	defer rows.Close()
	...
	if err := rows.Err(); err != nil {
		return nil, 0, fmt.Errorf("%s: rows scan error: %w", op, err)
	}

	var count int64
	if err = tx.QueryRow(`SELECT COUNT(*) FROM subscriptions;`).Scan(&count); err != nil {
		return nil, 0, fmt.Errorf("%s: failed to count rows: %w", op, err)
	}

	if err = tx.Commit(); err != nil {
		return nil, 0, fmt.Errorf("%s: failed to commit: %w", op, err)
	}
...
func (s *Storage) RangePrice(start_date time.Time, end_date time.Time, service_name string, user_id string) (uint64, error) {
	...
	FROM (
    	...
    	CASE
      		WHEN e < s THEN 0
      		ELSE (
        		(EXTRACT(YEAR FROM age(date_trunc('month', e) + interval '1 month',
                               			date_trunc('month', s)))::int * 12)
        		+ EXTRACT(MONTH FROM age(date_trunc('month', e) + interval '1 month',
                                 		date_trunc('month', s)))::int
      		)
    		END AS months_count
  		FROM (
    		SELECT
      		GREATEST(start_date, $1::date) AS s,
      		LEAST(COALESCE(end_date, $2::date), $2::date) AS e,
      		price
    	FROM subscriptions
